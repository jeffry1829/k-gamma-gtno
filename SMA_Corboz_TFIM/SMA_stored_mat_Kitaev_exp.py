...
torch.set_num_interop_threads(128)  # Inter-op parallelism
torch.set_num_threads(128)  # Intra-op parallelism
...
state = read_ipeps(args.statefile, global_args=cfg.global_args)
ENVfilenameC = Path(args.statefile).stem+"ENVC"+"chi" + \
    str(args.chi)+"size"+str(args.size)+".pt"
ENVfilenameT = Path(args.statefile).stem+"ENVT"+"chi" + \
    str(args.chi)+"size"+str(args.size)+".pt"
...
# print ("E_lowest_ex=",(Es[0]/64-4*energy_per_site.item().real))
# print ("ALL_E_lowest_ex=",(Es/64-4*energy_per_site.item().real))
print("E_lowest_ex=", Es[0].real-energy_per_site.item().real)
print("ALL_E_lowest_ex=", Es.real-energy_per_site.item().real)
...
# Es = Es.real
Es = (2*args.size+2)*(2*args.size+1)*(Es-4*energy_per_site.item().real)
