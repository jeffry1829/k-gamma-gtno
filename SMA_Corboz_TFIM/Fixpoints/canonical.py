...
def safesign(x):
    ...
    for i in range(len(x)):
        # print(x[i])
        newx[i] = (1 if np.abs(x[i]) <= 1e-10 else np.sign(x[i].real))
    return newx
...
def leftorth(A, C=None, tol=1e-12, maxiter=100):
    ...
    # Find the fixed point of ∑ᵢAⁱ⊗Aⁱ
    e, rho = eigs(TsfA, k=1, tol=tol, maxiter=1)
    rho = rho.reshape(D, D)
    ...
    while norm(C-R) > tol and numiter < maxiter:
        # G = R
        mixTsf = ncon([A, A.conj()], [[-1, 1, -3], [-2, 1, -4]]
                      ).reshape(D**2, D**2)
        C = eigs(mixTsf, v0=R, k=1, which='LM',
                 tol=tol, maxiter=1)[1].reshape(D, D)
        _, C = qrpos(C)
