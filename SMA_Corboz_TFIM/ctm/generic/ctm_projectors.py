...
def ctm_get_projectors_4x4(direction, coord, state, env, ctm_args=cfg.ctm_args,
                           global_args=cfg.global_args, diagnostics=None, applyHami=False, sameenv=False, *Hamis):
    r"""
    ...
    if direction == (0, -1):
        R, Rt = halves_of_4x4_CTM_MOVE_UP(
            coord, state, env, mode, verbosity, applyHami, sameenv, *Hamis)
    elif direction == (-1, 0):
        R, Rt = halves_of_4x4_CTM_MOVE_LEFT(
            coord, state, env, mode, verbosity, applyHami, sameenv, *Hamis)
    elif direction == (0, 1):
        R, Rt = halves_of_4x4_CTM_MOVE_DOWN(
            coord, state, env, mode, verbosity, applyHami, sameenv, *Hamis)
    elif direction == (1, 0):
        R, Rt = halves_of_4x4_CTM_MOVE_RIGHT(
            coord, state, env, mode, verbosity, applyHami, sameenv, *Hamis)
    else:
...
def ctm_get_projectors_from_matrices(R, Rt, chi, ctm_args=cfg.ctm_args,
    ...
    if ctm_args.projector_svd_method == 'DEFAULT' or ctm_args.projector_svd_method in ['GESDD', 'GESDD_CPU']:
        ...
        raise (
            f"Projector svd method \"{cfg.ctm_args.projector_svd_method}\" not implemented")
    # print("Doing QR for ctm projectors")
    # R = torch.linalg.qr(R.permute(1, 0))[1].permute(1, 0)
    # Rt = torch.linalg.qr(Rt.permute(1, 0))[1].permute(1, 0)
    #  SVD decomposition
    ...
    # print("RRt_approx norm:", torch.norm(RRt_approx))
    # print("RRt norm - RRt_approx norm:", torch.norm(RRt)-torch.norm(RRt_approx))

    # print((R@U.conj()@S_sqrt.diag().to(
    #     cfg.global_args.torch_dtype)@S_sqrt.diag().to(
    #     cfg.global_args.torch_dtype)@V.t()@Rt.t())[0:6, 0:6])

    # print("Pt@P", contract(Pt, P, ([1], [1]))[0:7, 0:7])
    # print("Pt@P", contract(Pt, P, ([1], [1])).shape)
...
#     # print("This is modified version of ctm_get_projectors_from_matrices")
#     if ctm_args.fwd_checkpoint_projectors:
...
#     # U2 = U.t()
#     # U2, S, V = truncated_svd(U2, chi)
#     # U = torch.zeros_like(U)
#     # U[:U2.size(0), :U2.size(1)] = U2
