...
def run(state, env, conv_check=None, ctm_args=cfg.ctm_args, global_args=cfg.global_args):
    ...
    for i in range(ctm_args.ctm_max_iter):
        ...
        for direction in ctm_args.ctm_move_sequence:
            diagnostics = {
                "ctm_i": i, "ctm_d": direction} if ctm_args.verbosity_projectors > 0 else None
            num_rows_or_cols = stateDL.lX if direction in [
                (-1, 0), (1, 0)] else stateDL.lY
        ...
        if conv_check is not None:
            ...
            converged, history = conv_check(
                state, env, history, ctm_args=ctm_args)
            if ctm_args.verbosity_ctm_convergence > 1:
                print(history)
            if converged:
...
def run_overlap(state1, state2, env, conv_check=None, ctm_args=cfg.ctm_args, global_args=cfg.global_args):
    ...
    for i in range(ctm_args.ctm_max_iter):
        ...
        if conv_check is not None:
            # evaluate convergence of the CTMRG procedure
            converged, history = conv_check(
                state1, state2, env, history, ctm_args=ctm_args)
            if ctm_args.verbosity_ctm_convergence > 1:
                print(history)
            if converged:
                if ctm_args.verbosity_ctm_convergence > 0:
...
def ctm_MOVE(direction, state, env, ctm_args=cfg.ctm_args, global_args=cfg.global_args,
    ...
    if ctm_args.projector_method == '4X4':
        ...
        raise ValueError("Invalid Projector method: " +
                         str(ctm_args.projector_method))

    # 0) extract raw tensors as tuple
    tensors = tuple(state.sites[key] for key in state.sites.keys()) \
        + tuple(env.C[key] for key in env.C.keys()) + \
        tuple(env.T[key] for key in env.T.keys())
    ...
    def ctm_MOVE_c(*tensors):
        ...
        state_loc = IPEPS(
            sites_loc, vertexToSite=state.vertexToSite, lX=state.lX, lY=state.lY)
        env_loc = ENV(env.chi)
        env_loc.C = dict(
            zip(env.C.keys(), tensors[len(state.sites):len(state.sites)+len(env.C)]))
        env_loc.T = dict(
            zip(env.T.keys(), tensors[len(state.sites)+len(env.C):]))
        ...
        for coord, site in state_loc.sites.items():
            ...
            if verbosity > 0:
                log.info("P,Pt RIGHT "+str(coord)+" P: " +
                         str(P[coord].size())+" Pt: "+str(Pt[coord].size()))
            if verbosity > 1:
                print(P[coord])
                print(Pt[coord])
        ...
        for coord in state_loc.sites.keys():
            if direction == (0, -1):
                ...
                nC1[coord], nC2[coord], nT[coord] = absorb_truncate_CTM_MOVE_LEFT(
                    coord, state_loc, env_loc, P, Pt, ctm_args)
            elif direction == (0, 1):
                nC1[coord], nC2[coord], nT[coord] = absorb_truncate_CTM_MOVE_DOWN(
                    coord, state_loc, env_loc, P, Pt, ctm_args)
            elif direction == (1, 0):
                nC1[coord], nC2[coord], nT[coord] = absorb_truncate_CTM_MOVE_RIGHT(
                    coord, state_loc, env_loc, P, Pt, ctm_args)
            else:
                raise ValueError("Invalid direction: "+str(direction))
            nC1[coord], nC2[coord], nT[coord] = move_normalize_c(
                nC1[coord], nC2[coord], nT[coord])
...
def absorb_truncate_CTM_MOVE_UP(coord, state, env, P, Pt, ctm_args=cfg.ctm_args):
    ...
    tensors = env.C[(coord, (1, -1))], env.T[(coord, (1, 0))], env.T[(coord, (0, -1))], \
        ...
        view(Pt[coord_shift_right], (env.chi, state.site(
            coord_shift_right).size(1+mode)**(mode+1), env.chi))
    if mode:
...
def absorb_truncate_CTM_MOVE_UP_c(*tensors):
    C1, T1, T, T2, C2, A, P2, Pt2, P1, Pt1, mode = tensors if len(
        tensors) == 11 else tensors+(None,)
...
def absorb_truncate_CTM_MOVE_LEFT(coord, state, env, P, Pt, ctm_args=cfg.ctm_args):
    ...
    tensors = env.C[(coord, (-1, -1))], env.T[(coord, (0, -1))], env.T[(coord, (-1, 0))], \
        ...
        view(Pt[coord_shift_up], (env.chi, state.site(
            coord_shift_up).size(2+mode)**(mode+1), env.chi))
    if mode:
        tensors += (torch.ones(1, dtype=torch.bool),)
...
def absorb_truncate_CTM_MOVE_DOWN(coord, state, env, P, Pt, ctm_args=cfg.ctm_args):
    ...
    tensors = env.C[(coord, (-1, 1))], env.T[(coord, (-1, 0))], env.T[(coord, (0, 1))], \
        ...
        view(Pt[coord_shift_left], (env.chi, state.site(
            coord_shift_left).size(3+mode)**(mode+1), env.chi))
    if mode:
        tensors += (torch.ones(1, dtype=torch.bool),)
...
def absorb_truncate_CTM_MOVE_RIGHT(coord, state, env, P, Pt, ctm_args=cfg.ctm_args):
    ...
    tensors = env.C[(coord, (1, 1))], env.T[(coord, (0, 1))], env.T[(coord, (1, 0))], \
        ...
        view(Pt[coord_shift_down], (env.chi, state.site(
            coord_shift_down).size(0+mode)**(mode+1), env.chi))
    if mode:
        tensors += (torch.ones(1, dtype=torch.bool),)
