...
def halves_of_4x4_CTM_MOVE_UP(coord, state, env, mode='sl', verbosity=0, applyHami=False, sameenv=False, *Hamis):
    r"""
    ...
    """
    # RU, RD, LU, LD
    tensors = c2x2_RU_t(coord, state, env, sameenv=sameenv) + c2x2_RD_t((coord[0], coord[1]+1), state, env, sameenv=sameenv) \
        + c2x2_LU_t((coord[0]-1, coord[1]), state, env, sameenv=sameenv) + \
        c2x2_LD_t((coord[0]-1, coord[1]+1), state, env, sameenv=sameenv)
    if mode in ['sl']:
    ...
    if ctm_args.fwd_checkpoint_halves:
        return checkpoint(halves_of_4x4_CTM_MOVE_UP_c, *tensors, applyHami=applyHami)
    else:
        return halves_of_4x4_CTM_MOVE_UP_c(*tensors, applyHami=applyHami)


def halves_of_4x4_CTM_MOVE_UP_t(coord, state, env):
    # RU, RD, LU, LD
    tensors = c2x2_RU_t(coord, state, env) + c2x2_RD_t((coord[0], coord[1]+1), state, env) \
        + c2x2_LU_t((coord[0]-1, coord[1]), state, env) + \
        c2x2_LD_t((coord[0]-1, coord[1]+1), state, env)
    return tensors


def halves_of_4x4_CTM_MOVE_UP_c(*tensors, applyHami=False):
    # C T T        C = C2x2_LU(coord+(-1,0))  C2x2(coord)
    # T A B(coord) T   C2x2_LD(coord+(-1,1))  C2x2(coord+(0,1))
    # T C D        T
    # C T T        C
    ...
    if tensors[-1]:  # mode
        if applyHami:
            RU = c2x2_RU_sl_c(*(tensors[0:4]+(conjA,)), applyHami=applyHami)
            ...
            #     contract(c2x2_LU_sl_c(*(tensors[8:12]+(conjA,)),applyHami=applyHami),c2x2_LD_sl_c(*(tensors[12:16]+(conjA,)),applyHami=applyHami),([0],[0]))
        else:
            return contract(c2x2_RU_sl_c(*tensors[0:4]), c2x2_RD_sl_c(*tensors[4:8]), ([1], [0])), \
                contract(c2x2_LU_sl_c(
                    *tensors[8:12]), c2x2_LD_sl_c(*tensors[12:16]), ([0], [0]))
    else:
        return contract(c2x2_RU_c(*tensors[0:4]), c2x2_RD_c(*tensors[4:8]), ([1], [0])), \
            contract(c2x2_LU_c(*tensors[8:12]),
                     c2x2_LD_c(*tensors[12:16]), ([0], [0]))


def halves_of_4x4_CTM_MOVE_LEFT(coord, state, env, mode='sl', verbosity=0, applyHami=False, sameenv=False, *Hamis):
    r"""
    :param coord: site for which to build two halfs of 2x2 subsystem embedded 
                  in environment 
    :type coord: tuple(int,int)
    ...
    """
    # LU, RU, LS, RD
    tensors = c2x2_LU_t(coord, state, env, sameenv) + c2x2_RU_t((coord[0]+1, coord[1]), state, env, sameenv) \
        + c2x2_LD_t((coord[0], coord[1]+1), state, env, sameenv) + \
        c2x2_RD_t((coord[0]+1, coord[1]+1), state, env, sameenv)
    if mode in ['sl']:
        tensors += (torch.ones(1, dtype=torch.bool),)
    else:
        tensors += (torch.zeros(1, dtype=torch.bool),)
    ...
    if applyHami:
        ...
        tensors += (torch.ones(1, dtype=torch.bool),)
    if ctm_args.fwd_checkpoint_halves:
        return checkpoint(halves_of_4x4_CTM_MOVE_LEFT_c, *tensors, applyHami=applyHami)
    else:
        return halves_of_4x4_CTM_MOVE_LEFT_c(*tensors, applyHami=applyHami)


def halves_of_4x4_CTM_MOVE_LEFT_t(coord, state, env):
    # LU, RU, LS, RD
    tensors = c2x2_LU_t(coord, state, env) + c2x2_RU_t((coord[0]+1, coord[1]), state, env) \
        + c2x2_LD_t((coord[0], coord[1]+1), state, env) + \
        c2x2_RD_t((coord[0]+1, coord[1]+1), state, env)
    return tensors


def halves_of_4x4_CTM_MOVE_LEFT_c(*tensors, applyHami=False):
    # C T        T C = C2x2_LU(coord)       C2x2(coord+(1,0))
    # T A(coord) B T   C2x2_LD(coord+(0,1)) C2x2(coord+(1,1))
    # T C        D T
    # C T        T C
    ...
    if tensors[-1]:  # mode
        if applyHami:
            LU = c2x2_LU_sl_c(*tensors[0:4]+(conjA,), applyHami=applyHami)
        ...
        return contract(c2x2_LU_c(*tensors[0:4]), c2x2_RU_c(*tensors[4:8]), ([1], [0])), \
            contract(c2x2_LD_c(*tensors[8:12]),
                     c2x2_RD_c(*tensors[12:16]), ([1], [1]))


def halves_of_4x4_CTM_MOVE_DOWN(coord, state, env, mode='sl', verbosity=0, applyHami=False, sameenv=False, *Hamis):
    r"""
    :param coord: site for which to build two halfs of 2x2 subsystem embedded 
                  in environment 
    :type coord: tuple(int,int)
    ...
    """
    # LD, LU, RD, RU
    tensors = c2x2_LD_t(coord, state, env, sameenv) + c2x2_LU_t((coord[0], coord[1]-1), state, env, sameenv) \
        + c2x2_RD_t((coord[0]+1, coord[1]), state, env, sameenv) + \
        c2x2_RU_t((coord[0]+1, coord[1]-1), state, env, sameenv)
    if mode in ['sl']:
        tensors += (torch.ones(1, dtype=torch.bool),)
    else:
        tensors += (torch.zeros(1, dtype=torch.bool),)
    ...
    if ctm_args.fwd_checkpoint_halves:
        return checkpoint(halves_of_4x4_CTM_MOVE_DOWN_c, *tensors, applyHami=applyHami)
    else:
        return halves_of_4x4_CTM_MOVE_DOWN_c(*tensors, applyHami=applyHami)


def halves_of_4x4_CTM_MOVE_DOWN_t(coord, state, env):
    # LD, LU, RD, RU
    tensors = c2x2_LD_t(coord, state, env) + c2x2_LU_t((coord[0], coord[1]-1), state, env) \
        + c2x2_RD_t((coord[0]+1, coord[1]), state, env) + \
        c2x2_RU_t((coord[0]+1, coord[1]-1), state, env)
    return tensors


def halves_of_4x4_CTM_MOVE_DOWN_c(*tensors, applyHami=False):
    # C T T        C = C2x2_LU(coord+(0,-1)) C2x2(coord+(1,-1))
    # T A        B T   C2x2_LD(coord)        C2x2(coord+(1,0))
    # T C(coord) D T
    # C T        T C
    ...
    if tensors[-1]:  # mode
        ...
        return contract(c2x2_LD_c(*tensors[0:4]), c2x2_LU_c(*tensors[4:8]), ([0], [0])), \
            contract(c2x2_RD_c(*tensors[8:12]),
                     c2x2_RU_c(*tensors[12:16]), ([0], [1]))


def halves_of_4x4_CTM_MOVE_RIGHT(coord, state, env, mode='sl', verbosity=0, applyHami=False, sameenv=False, *Hamis):
    r"""
    :param coord: site for which to build two halfs of 2x2 subsystem embedded 
                  in environment 
    :type coord: tuple(int,int)
    ...
    """
    # RD, LD, RU, LU
    tensors = c2x2_RD_t(coord, state, env, sameenv) + c2x2_LD_t((coord[0]-1, coord[1]), state, env, sameenv) \
        + c2x2_RU_t((coord[0], coord[1]-1), state, env, sameenv) + \
        c2x2_LU_t((coord[0]-1, coord[1]-1), state, env, sameenv)
    if mode in ['sl']:
        tensors += (torch.ones(1, dtype=torch.bool),)
    else:
        tensors += (torch.zeros(1, dtype=torch.bool),)
    ...
    if ctm_args.fwd_checkpoint_halves:
        return checkpoint(halves_of_4x4_CTM_MOVE_RIGHT_c, *tensors, applyHami=applyHami)
    else:
        return halves_of_4x4_CTM_MOVE_RIGHT_c(*tensors, applyHami=applyHami)
...
def halves_of_4x4_CTM_MOVE_RIGHT_t(coord, state, env):
    ...
    tensors = c2x2_RD_t(coord, state, env) + c2x2_LD_t((coord[0]-1, coord[1]), state, env) \
        + c2x2_RU_t((coord[0], coord[1]-1), state, env) + \
        c2x2_LU_t((coord[0]-1, coord[1]-1), state, env)
    return tensors


def halves_of_4x4_CTM_MOVE_RIGHT_c(*tensors, applyHami=False):
    # C T T        C = C2x2_LU(coord+(-1,-1)) C2x2(coord+(0,-1))
    # T A B        T   C2x2_LD(coord+(-1,0))  C2x2(coord)
    # T C D(coord) T
    # C T T        C
    ...
    if tensors[-1]:  # mode
        ...
        return contract(c2x2_RD_c(*tensors[0:4]), c2x2_LD_c(*tensors[4:8]), ([1], [1])), \
            contract(c2x2_RU_c(*tensors[8:12]),
                     c2x2_LU_c(*tensors[12:16]), ([0], [1]))
...
def c2x2_LU(coord, state, env, mode='dl', verbosity=0):
    ...
    if verbosity > 0:
        print("C2X2 LU "+str(coord)+"->" +
              str(state.vertexToSite(coord))+" (-1,-1)")
    if verbosity > 1:
        print(C2x2)
...
def c2x2_LU_sl_c(*tensors, applyHami=False):
    if applyHami:
        C, T1, T2, a, conjA = tensors
        ...
        C2x2 = contract(C2x2, conjA, ([0, 3], [1, 2]))
        ...
        # # |         |
        # # 1         3,6
        # C2x2 = contiguous(permute(C2x2,(2,5,3,6,4,7,0,1)))
        # C2x2 = view(C2x2,(a.size(0),a.size(0),a.size(3)**2,a.size(4)**2,T1.size(2),T2.size(1)))
        # # C---------T1--4
        # # |         |
        # # |         |/0,1
        # # T2-------a*a--3
        ...
        if not mode:
            ...
            C2x2 = view(C2x2, (T2.size(1)*(a.size(3)**2),
                        T1.size(2)*(a.size(4)**2)))
        else:
...
def c2x2_RU_sl_c(*tensors, applyHami=False):
    if applyHami:
        C, T1, T2, a, conjA = tensors
        ...
        #     3         |
        #               |
        #    5-a     0--T(1)         phys in A: 4
        #      6        1
        C2x2 = contract(C2x2, conjA, ([0, 3], [4, 1]))
        ...
        # # 1---T(2)------C
        # #               |
        # #  3,6-a*a      T(1)        phys in A: 2
        # #      4,7      0           phys in conj(A): 5
        # C2x2 = contiguous(permute(C2x2,(2,5,3,6,4,7,0,1)))
        # # 7---T(2)------C
        # #               |
        # #  2,3-a*a      T(1)        phys in A: 0
        # #      4,5      6           phys in conj(A): 1
        # C2x2 = view(C2x2,(a.size(0),a.size(0),a.size(2)**2,a.size(3)**2,T1.size(2),T2.size(0)))
        # # 5---T(2)------C
        # #               |
        # #    2-a*a      T(1)        phys in A: 0
        # #      3        4           phys in conj(A): 1
        return C2x2
    else:
        C, T1, T2, a, mode = tensors if len(tensors) == 5 else tensors+(None,)
        # 0--C
        ...
        if not mode:
            ...
            # reshape (124)(035)->01
            C2x2 = contiguous(permute(C2x2, (1, 2, 4, 0, 3, 5)))
            C2x2 = view(C2x2, (T2.size(0)*(a.size(2)**2),
                        T1.size(2)*(a.size(3)**2)))
        else:
            #    1--T2------C
            #       |       |
            #   2,5\|       |
...
def c2x2_RD_sl_c(*tensors, applyHami=False):
    if applyHami:
        C, T1, T2, a, conjA = tensors
        ...
        C2x2 = contract(C2x2, conjA, ([0, 3], [3, 4]))
        # permute 01234567->13604725
        # reshape (136)(047)25->0123
        C2x2 = contiguous(permute(C2x2, (1, 3, 6, 0, 4, 7, 2, 5)))
        ...
        # #    0--T1------C
        # C2x2 = contiguous(permute(C2x2,(2,5,3,6,4,7,0,1)))
        # C2x2 = view(C2x2,(a.size(0),a.size(0),a.size(1)**2,a.size(2)**2,T1.size(1),T2.size(0)))
        # #       2       5
        # #    3-a*a------T2
        # #   0,1/|       |
        # #       |       |
        ...
        if not mode:
            ...
            # reshape (124)(035)->01
            C2x2 = contiguous(permute(C2x2, (1, 2, 4, 0, 3, 5)))
            C2x2 = view(C2x2, (T2.size(0)*(a.size(1)**2),
                        T1.size(1)*(a.size(2)**2)))
        else:
            #       3,6     1
            #  4,7-a*a------T2
            #   2,5/|       |
...
def c2x2_LD_sl_c(*tensors, applyHami=False):
    if applyHami:
        C, T1, T2, a, conjA = tensors
        ...
        C2x2 = contract(C2x2, conjA, ([1, 2], [2, 3]))
        ...
        # # C--------T2--1
        # C2x2 = contiguous(permute(C2x2,(2,5,3,6,4,7,0,1)))
        # C2x2 = view(C2x2,(a.size(0),a.size(0),a.size(1)**2,a.size(4)**2,T1.size(0),T2.size(2)))
        # # 4        2
        # # T1------a*a--3
        # # |        |\0,1
        # # |        |
        ...
        if not mode:
            ...
            C2x2 = view(C2x2, (T1.size(0)*(a.size(1)**2),
                        T2.size(2)*(a.size(4)**2)))
        else:
            # 0        3,6
            # T1------a*a--4,7
            # |        |\2,5
