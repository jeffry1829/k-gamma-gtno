...
def Create_Localsite_Hami_Env(state, stateDL, B_grad, env, lam, Hx, Hy, Honsite, Id, kx, ky,
        ...
        for direction in cfg.ctm_args.ctm_move_sequence:
            if direction == (0, -1):
                ...
                if (MultiGPU):
                    ...
                    for key, site in ACstate.sites.items():
                        ACstate.sites[key] = site.to(local_device)
                    ACstate.device = local_device
                ...
                for j in range(2*args.size+2):
                    ...
                    T_down[(j)] = T_down[(j)].to(local_device)
                ...
                C_left["up"] = C_left["up"].to(local_device)
                vec_coord = (-args.size, args.size+1)
                new_coord = state.vertexToSite(
                    (coord[0]+vec_coord[0], coord[1]+vec_coord[1]))
                if "down" not in C_left:
                    C_left["down"] = env.C[(new_coord, (-1, 1))].clone()
                C_left["down"] = C_left["down"].to(local_device)
                for j in range(2*args.size+2):
                    ...
                    T_left[(j)] = T_left[(j)].to(local_device)
                ...
                C_right["up"] = C_right["up"].to(local_device)
                vec_coord = (args.size+1, args.size+1)
                new_coord = state.vertexToSite(
                    (coord[0]+vec_coord[0], coord[1]+vec_coord[1]))
                if "down" not in C_right:
                    C_right["down"] = env.C[(new_coord, (1, 1))].clone()
                C_right["down"] = C_right["down"].to(local_device)
                for j in range(2*args.size+2):
                    ...
                    T_right[(j)] = T_right[(j)].to(local_device)
    ...
    for key, site in ACstate.sites.items():
        ACstate.sites[key] = site.to(cfg.global_args.device)
    ACstate.device = cfg.global_args.device
    for key, site in env.C.items():
