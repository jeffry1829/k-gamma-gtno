...
if args.statefile == "KitaevLG.json":
    ...
    energy_per_site = energy_f(
        state, env, kx=args.Jx, ky=args.Jy, kz=args.Jz, h=args.h)
...
if len(state.sites) == 1:
    ...
    if (args.NormMat == True):
        ...
        C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right = Create_Norm_Env(state, stateDL, B_grad, env, lam, kx, ky,
                                                                                       C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right, args, firsttime=True, lasttime=True, ACstateDL=ACstateDL)
        ...
        for ii in range(elemsize):
            ...
            with torch.cuda.stream(streams[ii]):
                # print(ii)
                NormMat[(...,)+tuple(loc)] = conj(torch.autograd.grad(Norm[(0, 0)]
                                                                      [tuple(loc)].real, B_grad, create_graph=False, retain_graph=True)[0])
                NormMat[(...,)+tuple(loc)] += 1j*conj(torch.autograd.grad(Norm[(0, 0)]
    ...
    Hx = II + mu * args.Jz*ZZ  # might not have use
    # Hx = -args.hx*Sx

    # Hy = IIII - mu * (args.Jy*IYYI + args.Jz*(IIZZ+IZZI)/4)
    # Hz = IIII - mu * (args.Jx*IXXI + args.Jz*(IIZZ+IZZI)/4)
    Hy = IIII + mu * (args.Jy*IYYI + args.Jz*(IIZZ+ZZII)/4)
    Hz = IIII + mu * (args.Jx*IXXI + args.Jz*(IIZZ+ZZII)/4)
    ...
    if (args.HamiMat == True):
        ...
        C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right = Create_Localsite_Hami_Env(state, stateDL, B_grad, env, lam,
                                                                                                 Hy, Hz, Hx, II, kx, ky, C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right, args, firsttime=True, lasttime=True, isOnsiteWorking=isOnsiteWorking,
                                                                                                 ACstate=ACstate, MultiGPU=args.MultiGPU)
        Hami = Create_Localsite_Hami(state, env, C_up, T_up, C_left, T_left, C_down,
                                     T_down, C_right, T_right, Hy, Hz, Hx, II, args, isOnsiteWorking=isOnsiteWorking)
        ...
        for ii in range(elemsize):
            ...
            with torch.cuda.stream(streams[ii]):
                # print(loc)
                HamiMat0[tuple(loc)] = conj(torch.autograd.grad(
                    Hami[(0, 0)][tuple(loc)].real, mu, create_graph=True, retain_graph=True)[0])
                HamiMat0[tuple(loc)] += 1j*conj(torch.autograd.grad(Hami[(0, 0)]
                                                                    [tuple(loc)].imag, mu, create_graph=True, retain_graph=True)[0])
                HamiMat[(...,)+tuple(loc)] = conj(torch.autograd.grad(HamiMat0[tuple(loc)
                                                                               ].real, B_grad, create_graph=False, retain_graph=True)[0])
                HamiMat[(...,)+tuple(loc)] += 1j*conj(torch.autograd.grad(
