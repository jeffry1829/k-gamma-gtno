        ...
        for direction in cfg.ctm_args.ctm_move_sequence:
            if direction == (0, -1):
                ...
                _, s, _ = torch.linalg.svd(
                    C_up["left"])
                print("C_up[\"left\"] direction:",
                      direction, "original s:", s)
                ...
                for i in range(args.size+1):
                    ...
                    for j in range(2*args.size+2):
                        ...
                        Pt1 = view(Pt[(i, coord_shift_right, direction)], (env.chi, stateDL.site(
                            coord_shift_right).size()[1], env.chi))
                        if j % 2 == 0:
                            if args.size != 0  and i == args.size and j == args.size and lasttime:
                            ...
                            elif args.size == 0  and i == args.size and j == args.size and lasttime:
                                # Guess nT=
                                #           |------------T3
                                #  1 chi---Pt2           2
                                #           |---0
