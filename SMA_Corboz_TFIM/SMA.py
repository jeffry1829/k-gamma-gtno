...
if len(state.sites) == 1:
    ...
    for ii in range(elemsize):
        ...
        NormMat[(...,)+tuple(loc)] += 1j*conj(torch.autograd.grad(Norm[(0, 0)][tuple(loc)].imag,
    ...
    for coord, A in ACstate.sites.items():
        dimsA = A.size()
        a = einsum('mabcd,me->eabcd', A, Id - mu * (Sz + args.hx*Sx/2))
        a = einsum('mabcd,me->eabcd', a, Id - mu * (Sz + args.hx*Sx/2))
        a = contiguous(einsum('mefgh,mabcd->eafbgchd', a, conj(base)))
        a = view(a, (dimsA[1]**2, dimsA[2]**2, dimsA[3]**2, dimsA[4]**2))
        ACsitesDL[coord] = a
    ...
    C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right = Create_Localsite_Hami_Env(state, stateDL, B_grad, env, lam,
                                                                                             Hz, Hy, Hx, Id, kx, ky, C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right, args, firsttime=True, lasttime=True, isOnsiteWorking=isOnsiteWorking, ACstateDL=ACstateDL)
    ...
    for ii in range(elemsize):
        ...
        HamiMat0[tuple(loc)] += conj(torch.autograd.grad(
            Hami[(0, 0)][tuple(loc)].imag, mu, create_graph=True, retain_graph=True)[0])
        HamiMat[(...,)+tuple(loc)] = 1j*conj(torch.autograd.grad(HamiMat0[tuple(loc)].real,
                                                                 B_grad, create_graph=False, retain_graph=True)[0])
        HamiMat[(...,)+tuple(loc)] += 1j*conj(torch.autograd.grad(HamiMat0[tuple(loc)].imag,
                                                                  B_grad, create_graph=False, retain_graph=True)[0])
        HamiMat0.detach_()
    t2 = time.time()
