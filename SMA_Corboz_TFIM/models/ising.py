...
class ISING():
    ...
    def get_h(self):
        ...
        Sx = s2.SP()+s2.SM()
        SxIdIdId = torch.einsum('ia,jb,kc,ld->ijklabcd',
                                Sx, s2.I(), s2.I(), s2.I())
        Sy = -(s2.SP()-s2.SM())*1j
        SySy = torch.einsum('ij,ab->iajb', Sy, Sy)
        SySyIdId = torch.einsum('ijab,klcd->ijklabcd', SySy, id2)

        hp = -SzSzIdId - \
            SzSzIdId.permute(0, 2, 1, 3, 4, 6, 5, 7) - \
            self.q*SzSzSzSz - self.hx*SxIdIdId - \
            SySyIdId - SySyIdId.permute(0, 2, 1, 3, 4, 6, 5, 7)

        return SzSz+SySy, SzSzSzSz, Sx, hp
...
class ISING_C4V():
    ...
    def eval_obs(self, state, env_c4v):
        ...
        with torch.no_grad():
            ...
            rdm2x2 = rdm_c4v.rdm2x2(state, env_c4v)
            obs["SzSzSzSz"] = torch.einsum(
                'ijklabcd,ijklabcd', rdm2x2, self.szszszsz)
    ...
    def eval_corrf_SS(self, state, env_c4v, dist):
        ...
        Sz0szR = corrf_c4v.corrf_1sO1sO(
            state, env_c4v, Sop_zxy[0, :, :], get_op(Sop_zxy[0, :, :]), dist)
        Sx0sxR = corrf_c4v.corrf_1sO1sO(
            state, env_c4v, Sop_zxy[1, :, :], get_op(Sop_zxy[1, :, :]), dist)
