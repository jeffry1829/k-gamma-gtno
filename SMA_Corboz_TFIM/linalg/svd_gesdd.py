...
class SVDGESDD_legacy(torch.autograd.Function):
    ...
    def backward(self, dU, dS, dV):
        ...
        if (M > NS):
            dA = dA + (torch.eye(M, dtype=dU.dtype, device=dU.device) -
                       U@Ut) @ (dU*safe_inverse(S)) @ Vt
        if (N > NS):
            dA = dA + (U*safe_inverse(S)) @ dV.t() @ (torch.eye(N,
                                                                dtype=dU.dtype, device=dU.device) - V@Vt)
...
class SVDGESDD(torch.autograd.Function):
    ...
    def v1_10_Fonly_backward(self, gu, gsigma, gv):
        ...
        if not (gu is None):
            ...
            if m > k:
                ...
                u_term = u_term + \
                    proj_on_ortho_u @ (gu * sigma_inv.unsqueeze(-2))
            u_term = u_term @ vh
        else:
        ...
        if not (gv is None):
            ...
            if n > k:
                ...
                v_term = v_term + \
                    sigma_inv.unsqueeze(-1) * (gvh @ proj_on_v_ortho)
            v_term = u @ v_term
        else:
    ...
    def v1_11_backward(self, gU, gS, gVh):
        ...
        gA = (UhgU * S.unsqueeze(-2) + S.unsqueeze(-1) * VhgV) * \
            (1./reg_preinv(E)) + gS.diag_embed()
        if is_complex:
            gA = gA + (UhgU.diagonal(0, -2, -1) *
                       (1./(2. * reg_preinv(S)))).diag_embed()
...
def test_SVDGESDD_legacy_random():
    ...
    for split_scale in [0.]:  # 10.0, 1.0, 0.1, 0.01,
        ...
        try:
            A.requires_grad_()
            assert (torch.autograd.gradcheck(SVDGESDD_legacy.apply,
                    (A, eps, None,), eps=1e-6, atol=1.0, rtol=1.0e-3))
