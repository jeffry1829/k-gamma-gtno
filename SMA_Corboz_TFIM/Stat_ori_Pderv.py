...
def Create_Stat_Env(state, stateDL, B_grad, env, lam, lamb, iden, S, S_rot, S_ori, S_exp, S_rot_exp, norm_factor, kx, ky, args, ACstateDL):
                ...
                for i in range(args.size+1):
                    ...
                    for j in range(2*args.size+2):
                        ...
                        if i == args.size and j == args.size:
                            ...
                            if (i+j) % 2 == 0:
                                Aket = contract(iden + lamb * torch.exp(1j*(kx*vec_coord[0]+ky*vec_coord[1])) * (
                                    S/norm_factor - S_exp*iden), Aket, ([0], [0]))
                            else:
                                Aket = contract(iden + lamb * torch.exp(1j*(kx*vec_coord[0]+ky*vec_coord[1])) * (
                                    S_rot/norm_factor - S_rot_exp*iden), Aket, ([0], [0]))
                            Aket2 = Aket.detach()
                            nT2 = nT.detach()
                            Aket = contract(
                                (S_ori/norm_factor - S_exp*iden), Aket, ([0], [0]))
