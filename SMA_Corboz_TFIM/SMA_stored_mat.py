...
for i in range(Es.shape[0]):
    ans = np.linalg.norm(((Bs[:, i]))@NormMat@(SxA))**2 +\
        np.linalg.norm(((Bs[:, i]))@NormMat@(SyA))**2 +\
        np.linalg.norm(((Bs[:, i]))@NormMat@(SzA))**2
    # ans = np.linalg.norm(np.conj(np.transpose(Bs_Ori[:, i]))@NormMat_Ori@(SxA_Ori))**2 +\
    #     np.linalg.norm(np.conj(np.transpose(Bs_Ori[:, i]))@NormMat_Ori@(SyA_Ori))**2 +\
    #     np.linalg.norm(np.conj
    #         (np.transpose(Bs_Ori[:, i]))@NormMat_Ori@(SzA_Ori))**2
