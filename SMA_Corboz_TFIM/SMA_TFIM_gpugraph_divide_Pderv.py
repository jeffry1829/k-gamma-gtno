...
Numpyfilename = args.datadir+Path(args.statefile).stem+"hx"+str(args.hx)+"q"+str(args.q) +\
...
if len(state.sites) == 1:
    ...
    if (args.HamiMat == True):
        ...
        C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right = Create_Localsite_Hami_Env(state, stateDL, B_grad, env, lam,
                                                                                                 H, H, Honsite, iden2, kx, ky, C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right, args, firsttime=True, lasttime=True, isOnsiteWorking=isOnsiteWorking,
