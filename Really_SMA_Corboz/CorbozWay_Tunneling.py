...
#Copied from SMA_Kitaev_withP_Correct_Model_gpugraph_divide.py
...
if args.UseVUMPSansazAC:
    ...
    # Find rectangle tensor (c.f. Vanderstraeten, L., Mariën, M., Verstraete, F. & Haegeman, J. Excitations and the tangent space of projected entangled-pair states. Phys. Rev. B 92, 201111 (2015).)
    # Up T -> Left Rect, Left T -> Up Rect, (Down T -> Right Rect, Right T -> Down Rect)
    ...
    def _zassenhaus(*Matrixs):
        ...
        return At
    leftcA = torch.einsum('abc,dcfg->adbfg', ACLEFT, stateDL.site((0, 0))).reshape(env.chi*args.bond_dim**2, env.chi*args.bond_dim**2, args.bond_dim**2)
    # now, leftcA=
    #           0
    #         leftcA 2
    #           1
    subtns = []
    for idx in range(args.bond_dim**2):
        A = leftcA[:,:,idx]
        B = ACLEFT[:,:,idx]
        C = torch.zeros([B.shape[0], A.shape[0]])
        X = sylvester(A, B, C)
        subtns.append(X)
    leftX = _zassenhaus(*subtns)

    upcA = torch.einsum('abc,befg->aefcg', ACUP, stateDL.site((0, 0))).reshape(env.chi*args.bond_dim**2, args.bond_dim**2, env.chi*args.bond_dim**2)
    # now, upcA=
    #      0 upcA 2
    #         1
    subtns = []
