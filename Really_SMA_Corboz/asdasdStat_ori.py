...
def Create_Stat_Env(state, stateDL, B_grad, env, P, Pt, lam, mu, iden, Hx, Hy, norm_factor, kx, ky, args):
                ...
                for i in range(args.size+1):
                    ...
                    for j in range(2*args.size+2):
                        ...
                        if i == args.size and j == args.size:
                            ...
                            Aket = contract(iden + mu * Hy, Aket, ([0],[0]))
                            Aket2 = Aket.detach()
                            nT2 = nT.detach()
                            # Aket = contract((S_ori/norm_factor - S_exp*iden), Aket, ([0],[0]))
                            ...
                            nT = contract(nT2, Aket2,([0,3],[2,1]))
                            nT = contract(nT, view(P1,(env.chi,dimsA[4],dimsA[4],env.chi)),([3,6],[0,1]))
                            tempT2 = contiguous(permute(nT, (1,3,2,0,4,5,6)))
                ...
                for i in range(args.size+1):
                    ...
                    for j in range(2*args.size+2):
                        ...
                        if i == args.size and j == args.size:
                            ...
                            Aket = state.site(new_coord) + lam * torch.exp(-1j*(kx*vec_coord[0]+ky*vec_coord[1])) * B_grad
                            Aket = contract(iden + mu * Hx, Aket, ([0],[0]))
                            Aket2 = Aket.detach()
                            nT2 = nT.detach()
                            # Aket = contract((S_ori/norm_factor - S_exp*iden), Aket, ([0],[0]))
                            ...
                            nT = contract(nT2, Aket2,([0,4],[1,2]))
                            nT = contract(nT, view(Pt2,(env.chi,dimsA[3],dimsA[3],env.chi)),([2,5],[0,1]))
                            tempT2 = contiguous(permute(nT, (1,3,0,2,5,4,6)))
...
def Create_Stat(state, env, B_grad, lam, kx, ky):
    Hami = dict()
    tn = None
    for coord in state.sites.keys():
        ...
        #    /  |  \
        #  0    1    2
        tn = contract(tn, env.T[coord, (-1,0)], ([0],[0]))
        # now,
        #          tn
        #       /  |  \
        #     口    0    1
        ...
        #  2   3
        tnD = tn.size()
        tn = view(contiguous(tn), (int(sqrt(tnD[0])),int(sqrt(tnD[0])),tnD[1],tnD[2],int(sqrt(tnD[3])),int(sqrt(tnD[3]))))
        # now,
        #          tn
        #       /  |  \
        #     口   0,1  2
        ...
        # tn = contract(tn, centre, ([3,0],[2,1]))
        tn = contract(tn, centre, ([4,0],[2,1]))
        # now,
        #          tn
        #       /  |    \
        #     口    0    1       4 is phys bond
        #   /  \    | \
        #  2    3   5   6
        TD = env.T[coord, (1,0)].size()
        tn = contract(tn, view(contiguous(env.T[coord, (1,0)]), \
            (TD[0],int(sqrt(TD[1])),int(sqrt(TD[1])),TD[2])), ([1,6],[0,1]))
        # now,
        #          tn
        #       /  |    \
        #     0    2    3       1 is phys bond
        # now,
        #          tn
        #       /  |    \
        #     口    0    口       3 is phys bond
        #   /  \    |   /  \
        #  1    2   4  5    6 
        tn = contract(tn, env.C[coord, (-1,1)], ([1],[0]))
        # now,
        #          tn
        #       /  |  \
        #     口    1    2      0 is phys bond
        #      \
        #       3
        # now,
        #          tn
        #       /   |    \
        #     口-1  0  4 -口       2 is phys bond
        #      \    |     |
        #       6   3     5
        TD = env.T[coord, (0,1)].size()
        tn = contract(tn, view(contiguous(env.T[coord, (0,1)]),(int(sqrt(TD[0])),\
            int(sqrt(TD[0])), TD[1], TD[2])), ([3,6], [0,2]))
        # now,
        #          tn
        #       /  |  \
        #     口    口    1      0 is phys bond
        #            \
        #             2
        # now,
        #          tn
        #       /   |    \
        #     口-1  0  3 -口       2 is phys bond
        #           | \   |
        #           5  6  4
        tn = contract(tn, env.C[coord, (1,1)], ([4,6],[0,1]))
        # now,
        #          tn
        #           |    
        #     口-1  0  3 -口       2 is phys bond
        ...
        #           4 
        tnD = tn.size()
        tn = permute(contiguous(tn), (2,0,1,4,3))
        Hami[coord] = tn
    return Hami
