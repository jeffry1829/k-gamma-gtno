...
do
    ...
    datadir="data/HsuKe/correcth_h_00_to_015_kz_25_free/KitaevAnyon_withP_Jx${Jx}Jy${Jy}Jz${Jz}h${h}chi${chi}size${_size}bonddim${bond_dim}dtype${_dtype}/"
    mkdir -p ${datadir}
    ...
    if [[ "$runSMA" == "True" ]]; then
        if [[ "$OnlyOnePoint" == "True" ]]; then
            ...
            # K->M
            for kx in $(seq ${trdLp1} ${_step} ${hL}); do
                ky=$((${L}-$kx))
