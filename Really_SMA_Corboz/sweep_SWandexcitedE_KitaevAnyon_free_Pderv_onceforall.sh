#!/bin/bash
#
# First parameter is the Kz
# Second parameter is the h
...
# Nineth parameter is the only one point
...
do
    ...
    datadir="$3/KitaevAnyon_Pderv_Jx${Jx}Jy${Jy}Jz${Jz}h${h}chi${chi}size${_size}bonddim${bond_dim}dtype${_dtype}/"
    mkdir -p ${datadir}
    ...
    if [[ "$runSMA" == "True" ]]; then
        if [[ "$OnlyOnePoint" == "True" ]]; then
            ...
            # Gamma->K
            for kx in $(seq 0 ${_step} ${trdtL}); do
                ky=$((2*$kx))
                echo "kx="${kx} "ky="${ky}
                if [ ! -f "${datadir}kx${kx}.0ky${ky}.0HamiMat.npy" -a $skipCalculatedPoints == "True" ]; then
                    echo "Not skipped"
            ...
            # K->M
            for kx in $(seq ${trdtLp1} ${_step} ${L}); do
                ky=$((${tL}-$kx))
    ...
    if [[ "$runStoredMat" == "True" ]]; then
        ...
        if [ "$answer" == "y" ]; then
            ...
            if [[ "$OnlyOnePoint" == "True" ]]; then
                ...
                --kx ${kx} --ky ${ky} \
                --Jx ${Jx} --Jy ${Jy} --Jz ${Jz} --h ${h} --datadir ${datadir} ${extra_flags} \
                --reuseCTMRGenv ${reuseCTMRGenv} --removeCTMRGenv ${removeCTMRGenv} --eig_size ${eig_size} --projDerv True
            else
                # Correct Path:
                # M->Gamma
                for kx in $(seq ${L} -${_step} 1); do
    ...
    if [[ "$runDraw" == "True" ]]; then
        #Draw the figure
        python -u graph.py ${datadir} True "h=${h} Kz=${Kz}" "[0, 3, 5, 6], [r'\$M(\pi,0)\$', r'\$\Gamma(0,0)\$',r'\$K(\pi,\frac{\pi}{2})\$', r'\$M(\pi,0)\$']"\
        "${eig_size}" "./data/figures/KitaevKz${Kz}_projDerv_D${bond_dim}chi${chi}L${L}h${h}"
    fi
