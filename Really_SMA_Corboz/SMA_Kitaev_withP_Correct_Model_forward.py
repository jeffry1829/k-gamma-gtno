...
parser.add_argument("--NormMat", type=str, default="False",
    ...
    if (args.HamiMat == True):
        ...
        with torch.autograd.graph.save_on_cpu(False):
            ...
            C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right = checkpoint(Create_Localsite_Hami_Env, state, stateDL, B_grad, env, lam,
                                                                                      Hx, Hy, Honsite, II, kx, ky, C_up, T_up, C_left, T_left, C_down, T_down, C_right, T_right, args, True, True, P, Pt, isOnsiteWorking, use_reentrant=True)
            ...
            Hami = checkpoint(Create_Localsite_Hami, state, env, C_up, T_up, C_left, T_left, C_down,
                              T_down, C_right, T_right, Hx, Hy, Honsite, II, args, isOnsiteWorking, use_reentrant=True)
            ...
            for ii in range(elemsize):
                ...
                with torch.cuda.stream(streams[ii]):
                    ...
                    # torch.autograd.backward(Hami[(0, 0)][tuple(loc)].real,
                    #                         create_graph=True, retain_graph=True)
                    Hami[(0, 0)][tuple(loc)].real.backward(
                        create_graph=True, retain_graph=True)
                    HamiMat0[tuple(loc)] = conj(mu.grad.clone())
                    mu.grad = None
                    # torch.autograd.backward(HamiMat0[tuple(loc)].real,
                    #                         B_grad, create_graph=False, retain_graph=False)
                    HamiMat0[tuple(loc)].real.backward()
                    HamiMat[(...,)+tuple(loc)] = conj(B_grad.grad.clone())
