...
temp = contract(torch.from_numpy(NormMat),conj(state_t),([1],[0]))
print ("<Norm>=", contract(temp,state_t,([0],[0])).item().real/(2*args.size+2)**2)

state_t = view(state.site((0,0)),(HamiMat.shape[0]))
temp = contract(torch.from_numpy(HamiMat),conj(state_t),([1],[0]))
...
eig_size = 4
u, s, vh = np.linalg.svd(NormMat)
u = u[:,:eig_size]
s = s[:eig_size]
vh = vh[:eig_size,:]
NormMat = np.matmul(u, np.matmul(np.diag(s), vh))

u, s, vh = np.linalg.svd(HamiMat)
u = u[:,:eig_size]
s = s[:eig_size]
vh = vh[:eig_size,:]
HamiMat = np.matmul(u, np.matmul(np.diag(s), vh))
