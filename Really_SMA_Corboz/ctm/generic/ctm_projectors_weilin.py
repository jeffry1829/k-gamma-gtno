...
def ctm_get_projectors_4x4(direction, coord, state, env, ctm_args=cfg.ctm_args,
    ...
    if direction == (0, -1):
        R, Rt = halves_of_4x4_CTM_MOVE_UP(
            coord, state, env, verbosity=verbosity)
    elif direction == (-1, 0):
...
def ctm_get_projectors_from_matrices(R, Rt, chi, ctm_args=cfg.ctm_args,
    ...
    # print("Is close:", torch.allclose(
    #     M, mm(U, mm(S.diag().to(cfg.global_args.torch_dtype), V.conj().t()))))
    # print("S.size():", S.size())
    # print("S:", S)
    ...
    # print("Is close:", torch.allclose(
    #     torch.eye(M.size()[0], dtype=cfg.global_args.torch_dtype,
    #               device=cfg.global_args.device),
    #     R@U.conj()@torch.reciprocal(S).diag().to(
    #         cfg.global_args.torch_dtype)@V.t()@Rt.t(),
    #     atol=1e-3, rtol=1e-3))
    # print((R@U.conj()@torch.reciprocal(S).diag().to(
    #     cfg.global_args.torch_dtype)@V.t()@Rt.t())[0:10, 0:10])
# S_sqrt.diag().to(cfg.global_args.torch_dtype)@S_sqrt.diag().to(cfg.global_args.torch_dtype)

    def P_Pt_c(*tensors):
        R, Rt, U, V, S_sqrt = tensors
        return mm(R, conj(U))@S_sqrt.diag().to(cfg.global_args.torch_dtype), mm(Rt, V)@S_sqrt.diag().to(cfg.global_args.torch_dtype)
