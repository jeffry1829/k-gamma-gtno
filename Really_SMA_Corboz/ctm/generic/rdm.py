...
def _sym_pos_def_matrix(rdm, sym_pos_def=False, verbosity=0, who="unknown", **kwargs):
    ...
    if sym_pos_def:
        with torch.no_grad():
            ...
            if D.min() < 0:
                log.info(
                    f"{who} max(diag(rdm)) {D.max()} min(diag(rdm)) {D.min()}")
...
def _sym_pos_def_rdm(rdm, sym_pos_def=False, verbosity=0, who=None,  **kwargs):
    ...
    rdm = _sym_pos_def_matrix(
        rdm, sym_pos_def=sym_pos_def, verbosity=verbosity, who=who)
...
def rdm1x1_dl(coord, state, env, operator=None, sym_pos_def=False, force_cpu=False, verbosity=0):
    ...
    dimsA = a_1layer.size()
    if operator == None:
        a = contiguous(einsum('mefgh,nabcd->eafbgchdmn',
                       a_1layer, conj(a_1layer)))
        a = view(a, (dimsA[1] ** 2, dimsA[2] ** 2, dimsA[3]
                 ** 2, dimsA[4] ** 2, dimsA[0], dimsA[0]))
    else:
        a = contiguous(einsum('mefgh,nm,nabcd->eafbgchd',
                       a_1layer, operator, conj(a_1layer)))
        a = view(a, (dimsA[1] ** 2, dimsA[2] **
                 2, dimsA[3] ** 2, dimsA[4] ** 2))
    ...
    if operator == None:
        rdm = _sym_pos_def_rdm(rdm, sym_pos_def=sym_pos_def,
...
def rdm1x1_sl(coord, state, env, operator=None, sym_pos_def=False, force_cpu=False, verbosity=0):
    ...
    names = tuple(x.strip()
                  for x in "C1, T1, T4, a_op, a*, C2, T2, C3, T3, C4".split(','))

    path, path_info = get_contraction_path(
        *contract_tn, names=names, path=None, who=who)
...
def rdm2x1_dl(coord, state, env, sym_pos_def=False, force_cpu=False, verbosity=0):
    ...
    a = einsum('mefgh,nabcd->eafbgchdmn',
               state.site(coord), conj(state.site(coord)))
    a = view(contiguous(a),
    ...
    if verbosity > 0:
        print("C2X2 LU " + str(coord) + "->" +
              str(state.vertexToSite(coord)) + " (-1,-1): " + str(C2x2_LU.size()))
    ...
    if verbosity > 0:
        print("C2X1 LD " + str(coord) + "->" +
              str(state.vertexToSite(coord)) + " (-1,1): " + str(C2x1_LD.size()))
    ...
    a = einsum('mefgh,nabcd->eafbgchdmn', state.site(shift_coord),
               conj(state.site(shift_coord)))
    a = view(contiguous(a),
             (dimsA[1] ** 2, dimsA[2] ** 2, dimsA[3] ** 2, dimsA[4] ** 2, dimsA[0], dimsA[0]))
...
def rdm2x1_sl(coord, state, env, sym_pos_def=False, force_cpu=False, unroll=False,
    ...
    names = tuple(x.strip(
    ) for x in "C1, T1, T4, a, a*, C4, T3, T1_x, C2_x, T2_x, a_x, a_x*, C3_x, T3_x".split(','))
    #
    # This (typical) strategy is optimal, when X >> D^2 >> phys_dim
    #
    # path=((2, 5), (0, 12), (3, 11), (2, 10), (1, 9), (0, 8), (2, 5), (1, 6), (3, 5), (2, 4), (1, 3), (0, 2), (0, 1))
    path, path_info = get_contraction_path(
        *contract_tn, names=names, path=None, unroll=unroll, who=who)
...
def rdm1x2_dl(coord, state, env, sym_pos_def=False, force_cpu=False, verbosity=0):
    ...
    if verbosity > 0:
        print("C1X2 RU " + str(coord) + "->" +
              str(state.vertexToSite(coord)) + " (1,-1): " + str(C1x2_RU.size()))
...
def rdm1x2_sl(coord, state, env, sym_pos_def=False, force_cpu=False, unroll=False,
    ...
    names = tuple(x.strip(
    ) for x in "C1, T1, T4, a, a*, T2, C2, T4_y, C4_y, T3_y, a_y, a_y*, C3_y, T2_y".split(','))
    #
    # This (typical) strategy is optimal, when X >> D^2 >> phys_dim
    #
...
def rdm2x2_NNN_11(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    vec = (1, 1)
    shift_coord = state.vertexToSite((coord[0] + vec[0], coord[1] + vec[1]))
    C2X2_RD = c2x2_RD(shift_coord, state, env,
                      mode='sl-open', verbosity=verbosity)
...
def rdm2x2_NNN_1n1_legacy(coord, state, env, sym_pos_def=False, force_cpu=False, verbosity=0):
    ...
    vec = (1, -1)
    shift_coord = state.vertexToSite((coord[0] + vec[0], coord[1] + vec[1]))
    C2X2_RU = c2x2_RU(shift_coord, state, env,
                      mode='sl-open', verbosity=verbosity)
...
def rdm2x2_NNN_1n1_oe(coord, state, env, sym_pos_def=False, force_cpu=False,
    ...
    T1_ny = T1_ny.view(T1_ny.size(0), a_ny.size(1),
                       a_ny.size(1), T1_ny.size(2))
    T1_xny = T1_xny.view(T1_xny.size(0), a_xny.size(1),
                         a_xny.size(1), T1_xny.size(2))
    T2_xny = T2_xny.view(T2_xny.size(0), a_xny.size(4),
                         a_xny.size(4), T2_xny.size(2))
    T2_x = T2_x.view(T2_x.size(0), a_x.size(4), a_x.size(4), T2_x.size(2))
    T3_x = T3_x.view(a_x.size(3), a_x.size(3), T3_x.size(1), T3_x.size(2))
    T3 = T3.view(a.size(3), a.size(3), T3.size(1), T3.size(2))
    T4 = T4.view(T4.size(0), T4.size(1), a.size(2), a.size(2))
    T4_ny = T4_ny.view(T4_ny.size(0), T4_ny.size(1),
                       a_ny.size(2), a_ny.size(2))
    ...
    contract_tn = C1_ny, [0, 1], T1_ny, [1, 2, 5, 36], T4_ny, [0, 15, 3, 6], a_ny, [4, 2, 3, 16, 37], a_ny.conj(), [4, 5, 6, 17, 38], \
        ...
        T2_x, [33, 28, 31, 26], C3_x, [26, 27], T3_x, [29, 32, 41, 27], a_x, [
            30, 34, 39, 29, 28], a_x.conj(), [30, 35, 40, 32, 31], [11, 22, 14, 25]
...
def rdm2x2_legacy(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    C2x2_LU = view(C2x2_LU, (T2.size(1) * a.size(2),
                   T1.size(2) * a.size(3), dimsA[0], dimsA[0]))
    if verbosity > 0:
        print("C2X2 LU " + str(coord) + "->" +
              str(state.vertexToSite(coord)) + " (-1,-1): " + str(C2x2_LU.size()))
    ...
    dimsA = state.site(shift_coord).size()
    a = contiguous(einsum('mefgh,nabcd->eafbgchdmn',
                   state.site(shift_coord), conj(state.site(shift_coord))))
    a = view(a, (dimsA[1] ** 2, dimsA[2] ** 2, dimsA[3]
             ** 2, dimsA[4] ** 2, dimsA[0], dimsA[0]))
    ...
    C2x2_RU = view(C2x2_RU, (T2.size(0) * a.size(1),
                   T1.size(2) * a.size(2), dimsA[0], dimsA[0]))
    if verbosity > 0:
        print("C2X2 RU " + str((coord[0] + vec[0], coord[1] + vec[1])) + "->" + str(shift_coord) + " (1,-1): " + str(
            C2x2_RU.size()))
    ...
    C2x2_RD = view(C2x2_RD, (T2.size(0) * a.size(0),
                   T1.size(1) * a.size(1), dimsA[0], dimsA[0]))
    ...
    C2x2_LD = view(C2x2_LD, (T1.size(0) * a.size(0),
                   T2.size(2) * a.size(3), dimsA[0], dimsA[0]))
    if verbosity > 0:
        print("C2X2 LD " + str((coord[0] + vec[0], coord[1] + vec[1])) + "->" + str(shift_coord) + " (-1,1): " + str(
            C2x2_LD.size()))
...
def rdm2x3_trglringex(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    T_10 = T_10.view([state.site((shift_coord)).size(3)]
                     * 2+[T_10.size(1), T_10.size(2)])
    # C2x2_LU--0
    # |
    # |     ->3,4    ->1,2
    # |/23->4,5   /->2,3  0,1->5,6
    ...
    # |     |       5,6->4
    # C2x2_LD-------T---7->5
    C2X2_LU = torch.tensordot(
        C2X2_LU, state.site(shift_coord), ([1, 5], [2, 3]))
    ...
    C2X2_LU = torch.tensordot(C2X2_LU, state.site(
        shift_coord).conj(), ([1, 4], [2, 3]))
    vec = (1, -1)
    shift_coord = state.vertexToSite((coord[0] + vec[0], coord[1] + vec[1]))
    ...
    T_11 = T_11.view(
        [T_11.size(0)]+[state.site((shift_coord)).size(1)]*2+[T_11.size(2)])
    ...
    #     8<-   2<-  |
    #          5<-7--C2x2_RD
    C2X2_RU = torch.tensordot(
        C2X2_RU, state.site(shift_coord), ([1, 3], [1, 4]))
    ...
    #      6<- 3        |
    #       9<-   3<-5--C2x2_RD
    C2X2_RU = torch.tensordot(C2X2_RU, state.site(
        shift_coord).conj(), ([1, 2], [1, 4]))
    vec = (1, 0)
    shift_coord = state.vertexToSite((coord[0] + vec[0], coord[1] + vec[1]))
    ...
    # 0,1 2,3 x
    rdm = torch.tensordot(
        C2X2_LU, C2X2_RU, ([0, 1, 2, 5, 8, 11, 7, 10], [0, 7, 10, 3, 4, 5, 8, 11]))
...
def rdm2x3_trglringex_compressed(coord, state, env, compressed_chi=None, sym_pos_def=False,
    ...
    if not compressed_chi:
        compressed_chi = env.chi
    # ----- building C2x2_LU ----------------------------------------------------
    vec = (0, -1)
    shift_coord = state.vertexToSite((coord[0] + vec[0], coord[1] + vec[1]))
    C2X2_LU = c2x2_LU(shift_coord, state, env, mode='sl', verbosity=verbosity)
    ...
    #       |_1 1_|
    vec = (1, -1)
    shift_coord_1n1 = state.vertexToSite(
        (coord[0] + vec[0], coord[1] + vec[1]))
    vec = (1, 0)
    ...
    T_10 = T_10.view([state.site(shift_coord_10).size(3)]
                     * 2+list(T_10.size())[1:])
    Pt_down = Pt_down.view(
        [T_10.size(3)]+[state.site(shift_coord_10).size(4)]*2+[Pt_down.size(1)])
    ...
    T_1n1 = T_1n1.view(
        [T_1n1.size(0)]+[state.site(shift_coord_1n1).size(1)]*2+[T_1n1.size(2)])
    Pt_up = Pt_up.view(
        [T_1n1.size(3)]+[state.site(shift_coord_1n1).size(4)]*2+[Pt_up.size(1)])
...
def rdm3x2_trglringex(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    T_1n1 = T_1n1.view(
        [T_1n1.size(0)]+[state.site(shift_coord_1n1).size(4)]*2+[T_1n1.size(2)])
    ...
    C2X2_LU = torch.einsum(C2X2_LU, [0, 1, 7, 10, 13, 14], T_1n1, [1, 2, 3, 4],
                           ...
                           state.site(shift_coord_1n1).conj(), [
        12, 10, 11, 6, 3],
    ...
    vec = (0, -1)
    shift_coord_0n1 = state.vertexToSite(
        (coord[0] + vec[0], coord[1] + vec[1]))
    T_0n1 = env.T[shift_coord_0n1, (-1, 0)]
    T_0n1 = T_0n1.view([T_0n1.size(0)]+[T_0n1.size(1)] +
                       [state.site(shift_coord_0n1).size(2)]*2)
    ...
    C2X2_RD = torch.einsum(C2X2_RD, [0, 4, 5, 6, 13, 14], T_0n1, [1, 4, 2, 3],
                           ...
                           state.site(shift_coord_0n1).conj(), [
        12, 10, 3, 6, 11],
...
def rdm3x2_trglringex_compressed(coord, state, env, compressed_chi=None, sym_pos_def=False,
    ...
    T_1n1 = T_1n1.view(
        [T_1n1.size(0)]+[state.site(shift_coord_1n1).size(4)]*2+[T_1n1.size(2)])
    P_right = P_right.view(
        [T_1n1.size(3)]+[state.site(shift_coord_1n1).size(3)]*2+[P_right.size(1)])
    ...
    T_0n1 = T_0n1.view([T_0n1.size(0)]+[T_0n1.size(1)] +
                       [state.site(shift_coord_0n1).size(2)]*2)
    P_left = P_left.view(
        [T_0n1.size(0)]+[state.site(shift_coord_0n1).size(1)]*2+[P_left.size(1)])
...
def norm_C4(coord, state, env):
    ...
    norm = torch.einsum(env.C[coord, (-1, -1)], [0, 1],
                        env.C[state.vertexToSite(
                            (coord[0]-1, coord[1])), (1, -1)], [1, 2],
                        env.C[state.vertexToSite(
                            (coord[0]-1, coord[1]-1)), (1, 1)], [2, 3],
...
def norm_3x3(coord, state, env, verbosity=0):
    ...
    E = corrf.apply_TM_1sO(coord, (0, 1), state, env, E,
                           op=None, verbosity=verbosity)
...
def _Lhalf_1x2(coord, state, env):
    ...
    # C4----T3--3->4
    T1 = env.T[coord, (0, -1)]
    Lhalf = torch.tensordot(T1.permute(
        0, 2, 1).contiguous(), Lhalf, ([0], [0]))
    return Lhalf
