'''
Copied from https://github.com/wangleiphy/tensorgrad/blob/master/tensornets/adlib/svd.py
...
class SVD(torch.autograd.Function):
    ...
    def backward(self, dU, dS, dV):
        ...
        if (M > NS):
            dA = dA + (torch.eye(M, dtype=dU.dtype,
                       device=dU.device) - U@Ut) @ (dU/S) @ Vt
        if (N > NS):
            dA = dA + (U/S) @ dV.t() @ (torch.eye(N,
