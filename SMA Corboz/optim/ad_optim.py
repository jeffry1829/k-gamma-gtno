...
def optimize_state(state, ctm_env_init, loss_fn, params, obs_fn=None, post_proc=None,
    ...
    #params2 = params + state.get_parameters()
    #params.append(state.sites[(0,0)][0,0,0,0,0])
    #params.append(state.sites[(0,0)][1,0,0,0,0])
    # for A in parameters: A.requires_grad_(True)
    for A in params: A.requires_grad_(True)
    # params2 = params + list(parameters)
    params2 = params
    
    optimizer = torch.optim.LBFGS(params2, max_iter=opt_args.max_iter_per_epoch, lr=opt_args.lr, \
    ...
    def closure():
        # 0) evaluate loss
        optimizer.zero_grad()
        loss, ctm_env, history, t_ctm, t_check = loss_fn(state, current_env[0], params, context)
        # print("loss : ", loss)
        ...
        if t_data["min_loss"] > t_data["loss"][-1]:
            ...
            # state.write_to_file(outputstatefile, symmetrize=False, normalize=False)
            state.write_to_file(outputstatefile, normalize=False)
            str1 = " ".join(str(e.item()) for e in params)
            # f=open(main_args.params_out+'.txt', 'w')
            f=open("test"+'.txt', 'w')
            f.write(str1)
            f.close()
        ...
        if obs_fn is not None:
            obs_fn(state, ctm_env, params, context)

        # 4) evaluate gradient
        t_grad0= time.perf_counter()
        # print("params1 :",parameters)
        ...
        #     print("\n\n params2 : \n\n", params2[0].grad)
        #     log_entry["grad_mag"]= [p.grad.norm().item() for p in params2]
        #     if opt_args.opt_log_grad: log_entry["grad"]= [p.grad.tolist() for p in params2]
        #     log.info(json.dumps(log_entry))
