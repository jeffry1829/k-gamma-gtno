...
##            params[i]+= noise * rand_t[0]
    ...
    bond_dim = args.bond_dim
    #[-0.0180-0.0805j, -0.0246+0.0409j, -0.5349+0.8605j, 0.7144-0.1709j, -0.3380+0.4929j, -1.0177+0.1390j, -0.2417+0.7808j, -0.9978+0.5322j]

    A=state_111()
    # A[0,0,0,0] = -0.2417+0.7808j
    # A[1,0,0,0] = -0.9978+0.5322j
    # a, b = to_bloch(A)
    # A[0,0,0,0] = torch.cos(a/2)
    # A[1,0,0,0] = np.exp(-1j*b)*torch.sin(a/2)
    ...
    # # A1[0,0,0,0] = params[-2]
    # # A1[1,0,0,0] = params[-1]
    A1[0,0,0,0] = torch.cos(a/2)
    A1[1,0,0,0] = np.exp(-1j*b)*torch.sin(a/2)
    print(A1[:,0,0,0])
    exit()
    # A2 = state_111()
    # A2[0,0,0,0] = params[-4]
    # A2[1,0,0,0] = params[-3]
    # A2[0,0,0,0] = torch.cos(a/2)
    # A2[1,0,0,0] = np.exp(1j*b)*torch.sin(a/2)
    ...
    if args.tiling in ["2SITE"]:
        ...
        B1 = state_111()
        # B1[0,0,0,0] = torch.cos(a/2)
        # B1[1,0,0,0] = np.exp(1j*b)*torch.sin(a/2)
        B2 = state_111()
        # B2[0,0,0,0] = torch.cos(a/2)
        # B2[1,0,0,0] = np.exp(1j*b)*torch.sin(a/2)
        # B1[0,0,0,0] = params[-6]
        # B1[1,0,0,0] = params[-5]
