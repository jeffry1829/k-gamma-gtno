...
parser.add_argument("--tiling", default="BIPARTITE", help="tiling of the lattice", \
    choices=["2SITE"])
parser.add_argument("--Efn", type=str, default="datas/anik/anik_E1_h0_25toCSL.txt", help="filename for energy")
parser.add_argument("--magfn", type=str, default="datas/anik/anik_mag1_h0_25toCSL.txt", help="filename for magnetization")
parser.add_argument("--csfn", type=str, default="datas/anik/anik_cs1_h0_25toCSL.txt", help="filename for correlation function")
parser.add_argument("--Wfn", type=str, default="datas/anik/anik_W1_h0_25toCSL.txt", help="filename for correlation function")
...
def main():
    ...
    praw=[0,-0.9,0,-0.9,0,-0.9,0,0,0,0,0,0,0, 5.3280 ,2.3564, 4.0969 , -0.7854] # AFM kitaev no LG no params
    ...
    Q = LG(torch.tensor(0,dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device))
    # Q = LG(params[13])
    A1 = state_001()
    ...
    for i in range(css.shape[0]):
        ...
        for j in range(num_params):
            ...
            params.append(torch.tensor(praw[j+1],dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device))
        ...
        def save(state, ctm_env_in, params):
            ...
            with open(csfn, "a") as f:
                np.savetxt(f, tmp.reshape(1, tmp.shape[0]))
            f.close() 
            tmp = np.asarray(W); tmp = np.insert(tmp, 0, h)
        ...
        for p in LGps:
            print("p = ", p)
            params[13] = torch.tensor(p,dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device)
