...
# -------- filenames --------
Efn = f"datas/kg_LG_zz_E1.txt"
magfn = f"datas/kg_LG_zz_mag1.txt"
csfn = f"datas/kg_LG_zz_cs1.txt"
# -------- filenames --------
...
def main():
    ...
    css = np.loadtxt("datas/kg_cs1.txt")
    for i in range(css.shape[0]):
        ...
        # for k in range(8):
        #     print(css[i,a[k]]+1j*css[i,b[k]])
        #     np.append(praw, css[i,a[k]]+1j*css[i,b[k]])
        # print(praw)
        ...
        for k in range(8):
            params.append(torch.tensor(css[i,a[k]]+1j*css[i,b[k]],dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device))

        zs = torch.tensor(np.linspace(1.0, 0, 4), device=cfg.global_args.device)
