...
##            params[i]+= noise * rand_t[0]
    ...
    single_A[0,0,0,0] = torch.tensor((1/(2*tmpb)))
    single_A[1,0,0,0] = torch.tensor(((1+1j)*tmpb))

    Dsingle_A = torch.zeros((model.phys_dim, bond_dim, bond_dim, bond_dim),\
                dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device)
    Dsingle_A[0,0,0,0] = torch.tensor((1/(2*tmpb)))
    Dsingle_A[1,0,0,0] = torch.tensor(((1-1j)*tmpb))
    ...
    double_Q =  torch.einsum('ijklm,abcdm->iajblcdk', Q, Q).reshape(model.phys_dim**2, model.phys_dim**2, 2, 2, 2, 2)
