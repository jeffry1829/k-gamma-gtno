...
# -------- filenames --------
Efn = f"datas/kg_E1.txt"
magfn = f"datas/kg_mag1.txt"
csfn = f"datas/kg_cs1.txt"
# -------- filenames --------
...
##            params[i]+= noise * rand_t[0]
        ...
        #praw = [0.3298, -0.7930, 0, 0.3409, 0, 0, 0, -0.0180-0.0805j, -0.0246+0.0409j, -0.5349+0.8605j, 0.7144-0.1709j, -0.3380+0.4929j, -1.0177+0.1390j, -0.2417+0.7808j, -0.9978+0.5322j]
        #praw = [0.3298, -0.7000, 0, 0.3009, 0, 0, 0, -0.0180-0.0805j, -0.0246+0.0409j, -0.5349+0.8605j, 0.7144-0.1709j, -0.3380+0.4929j, -1.0177+0.1390j, -0.2417+0.7808j, -0.9978+0.5322j]
        ...
        # praw = [np.cos(0.146), np.sin(0.146), 0, 0, 0, 0, 0, 1, 2.3564, 5.3280, 2.3564, 5.3280, 2.3564, 5.3280, 2.3564, 5.3280]
        praw =  [0.43772039655102224, -0.8226906250029886, -0.03364959740129645, 0.3228885974360782, 0.041877485181152564, -0.09836847152808323, -0.05658808838622629, (-0.020276949177556583-0.08749222706437176j), (-0.02396445873412625+0.03838319313721153j), (-0.5396273074246298+0.8716723665566873j), (0.6986831159506661-0.16460282013398675j), (-0.31812019683775306+0.42996600442133925j), (-1.0499297674215902+0.15642868849855596j), (-0.23444645111795817+0.7652910111861843j), (-1.009410679213222+0.536369677607452j)]
    ...
    #python optim_gamma_LG.py --tiling=2SITE --params_out=test --GLOBALARGS_dtype=complex128 --CTMARGS_ctm_conv_tol=1e-2 --OPTARGS_lr=1.0 --GLOBALARGS_device=cuda:0
    ...
    # G1A = torch.einsum('ijklm,jabc->ikalbmc', Q, G1A).reshape(model.phys_dim, 8, 8, 8)
    G2A = torch.einsum('ijklm,jabc->ikalbmc', G2, A2).reshape(model.phys_dim, 4, 4, 4)
    # G2A = torch.einsum('ijklm,jabc->ikalbmc', Q, G2A).reshape(model.phys_dim, 8, 8, 8)
    # A_= torch.einsum('iklm,akde->ialmde', G1A, G2A).reshape(model.phys_dim*model.phys_dim, 8, 8, 8, 8)
    ...
    if args.tiling in ["2SITE"]:
        ...
        # B1[1,0,0,0] = torch.exp(-1cj*params[-6])*torch.sin(params[-5]/2)
        B1[0,0,0,0] = params[-6]
        B1[1,0,0,0] = params[-5]
        B2 = state_111()
        ...
        # G1B = torch.einsum('ijklm,jabc->ikalbmc', Q, G1B).reshape(model.phys_dim, 8, 8, 8)
        G2B = torch.einsum('ijklm,jabc->ikalbmc', G2_, B2).reshape(model.phys_dim, 4, 4, 4)
        # G2B = torch.einsum('ijklm,jabc->ikalbmc', Q, G2B).reshape(model.phys_dim, 8, 8, 8)
    ...
    st = time.time() 
    loss = energy_f(state_sym, ctm_env, phi=0.5*np.pi)
    print(model.get_m())
    end = time.time()
    ...
    phis = np.pi*np.linspace(0.5, 0.0, 40)
    ...
    for phi in phis:
        ...
        def loss_fn(state, ctm_env_in, params, opt_context):
            ...
            if args.tiling in ["2SITE"]:
                ...
                G2B = torch.einsum('ijklm,jabc->ikalbmc', G2_, B2).reshape(model.phys_dim, 4, 4, 4)
                # G2B = torch.einsum('ijklm,jabc->ikalbmc', Q, G2B).reshape(model.phys_dim, 8, 8, 8)
                # B_= torch.einsum('iklm,akde->ialmde', G1B, G2B).reshape(model.phys_dim*model.phys_dim, 8, 8, 8, 8)
                B_= torch.einsum('iklm,akde->ialmde', G1B, G2B).reshape(model.phys_dim*model.phys_dim, 4, 4, 4, 4)
                B_= B_/B_.norm()
        ...
        def save(state, ctm_env_in, params):
            ...
            for i in range(num_params):
                if i < 7:
                    plist.append(params[i].cpu().clone().detach().item().real)
                else:
                    plist.append(params[i].cpu().clone().detach().item())
            print("params : ", plist)
            # for ele in Es:
            #     ele = ele.cpu()
