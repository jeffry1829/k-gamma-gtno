...
def main():
    ...
    num_params = 16 # 7 for GTNO 8=2*4 for states
...
#python optim_gamma_noLG.py --tiling=2SITE --params_out=test --GLOBALARGS_dtype=complex128 --CTMARGS_ctm_conv_tol=1e-3 --OPTARGS_lr=1.0 --GLOBALARGS_device=cuda:0
    ...
    phis = np.pi*np.linspace(0.0, 0.5, 20)
    for phi in phis:
        print("===================")
        print("phi = ", phi)
        ...
        @torch.no_grad()
        def loss_fn(params, lossargs):
            # state, ctm_env_in, opt_context = lossargs
            opt_context = lossargs
            ...
            # # 1) compute environment by CTMRG
            # ctm_env_out, *ctm_log = ctmrg.run(state_sym, ctm_env_in, conv_check=ctmrg_conv_rdm2x1)
            ctm_env_out, *ctm_log = ctmrg.run(state_sym, ctm_env, conv_check=ctmrg_conv_rdm2x1)
            loss = energy_f(state_sym, ctm_env_out, phi = phi)
            model.get_m()
            print("Energy = ", loss.item())
            for p in params:
        ...
        minimize(loss_fn, params, method='L-BFGS-B', options={'disp': True, 'maxiter': 1000, 'maxfun': 1000})
