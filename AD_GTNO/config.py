...
def get_args_parser():
    ...
    parser.add_argument("--omp_cores", type=int, default=1,
                        help="number of OpenMP cores")
    parser.add_argument("--instate", default=None, help="Input state JSON")
    parser.add_argument("--params_in", default=None,
                        help="Input c parameters txt")
    parser.add_argument("--params_out", default=None,
                        help="Output c parameters txt")
    parser.add_argument("--instate_noise", type=float, default=0.,
                        help="magnitude of noise added to the trial \"instate\"")
    parser.add_argument("--ipeps_init_type", default="RANDOM",
                        help="initialization of the trial iPEPS state")
    parser.add_argument("--out_prefix", default="output",
                        help="Output files prefix")
    parser.add_argument("--bond_dim", type=int, default=1,
                        help="iPEPS auxiliary bond dimension")
    parser.add_argument("--chi", type=int, default=20,
                        help="environment bond dimension")
    parser.add_argument("--opt_max_iter", type=int,
                        default=100, help="maximal number of epochs")
    parser.add_argument("--opt_resume", type=str, default=None,
                        help="file with checkpoint to resume")
    parser.add_argument("--opt_resume_override_params", type=bool, default=False,
                        help="override optimizer parameters stored in checkpoint")
    ...
    for c in configs:
        ...
        for x in c_list:
            if isinstance(getattr(c, x), bool):
                # default is False
                if not getattr(c, x):
                    parser.add_argument("--"+group_prefix+x,
                                        action='store_true')
                # default is True
                else:
                    parser.add_argument(
                        "--"+group_prefix+"no_"+x, action='store_false', dest=group_prefix+x)
            else:
                parser.add_argument(
                    "--"+group_prefix+x, type=type(getattr(c, x)), default=getattr(c, x))
...
def configure(parsed_args):
    ...
    # set up logger
    logging.basicConfig(filename=main_args.out_prefix +
