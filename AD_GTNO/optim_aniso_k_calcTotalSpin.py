...
parser.add_argument("--Wfn", type=str, default="datas/anik/anik_W1_h0_25toCSL.txt",
                    help="filename for correlation function")
parser.add_argument("--TSfn", type=str, default="datas/anik/anik_TotalSpin1_h0_25toCSL.txt",
                    help="filename for correlation function")
...
Wfn = args.Wfn
TSfn = args.TSfn
npyfn = "datas/aniksdg_noLG_sweepLG_TS/Kz{}h{}.npy".format(args.Kz, args.h)
...
os.makedirs(os.path.dirname(TSfn), exist_ok=True)
os.makedirs(os.path.dirname(npyfn), exist_ok=True)
...
def main():
    ...
    praw = [0, -0.9, 0, -0.9, 0, -0.9, 0, 0, 0, 0, 0, 0, 0,
            1, 5.3280, 2.3564, 4.0969, -0.7854]  # AFM kitaev
    ...
    # Q = LG(torch.tensor(1,dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device))
    Q = LG(params[13])
    A1 = state_001()
    ...
    for i in range(css.shape[0]):
        ...
        def save(state, ctm_env_in, params):
            ...
            W = model.get_Wp()
            W = [W]
            TotalSpin = model.get_TotalSpin()

            print("Mags = ", ms)
            print("E_total = ", Es[-1])
            print("W = ", W)
            print("TotalSpin = ", TotalSpin)
            ...
            #     np.savetxt(f, tmp.reshape(1, tmp.shape[0]))
            # f.close()
            # tmp = np.asarray(TotalSpin)
            # tmp = np.insert(tmp, 0, h)
            # with open(TSfn, "a") as f:
            #     np.savetxt(f, tmp.reshape(1, tmp.shape[0]))
            # f.close()
            npyfn = "datas/aniksdg_noLG_sweepLG_TS/Kz{}cnt{}.npy".format(args.Kz, i)
            # with open(npyfn, "wb") as f:
            #     np.save(npyfn, torch.einsum("iabcd,jefgh->ijaebfcgdh",state.sites[(0,0)],state.sites[(1,0)]).reshape(model.phys_dim**4,16,16,16,16).cpu().numpy())
            # f.close()
            np.save(npyfn, state.sites[(0,0)].reshape(model.phys_dim*model.phys_dim, 4, 4, 4, 4).cpu().numpy())
            return 0
        cfg.opt_args.lr = 0.5
        ...
        loss_fn(state_sym, ctm_env, params, [])
        save(state_sym, ctm_env, params)
    return 0
