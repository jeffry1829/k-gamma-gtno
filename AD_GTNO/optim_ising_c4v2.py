#python optim_ising_c4v2.py --bond_dim 1 --chi 16 --seed 1234 --hx 3.0 --CTMARGS_fwd_checkpoint_move --OPTARGS_tolerance_grad 1.0e-8 --out_prefix ex-hx30D4 --params_out paramsh30D4 --opt_max_iter 1000 --instate ex-hx30D4_state.json --params_in paramsh30D4
import context
import torch
import argparse
...
##            params[i]+= noise * rand_t[0]
    ...
    if len(params)==9:
        for i in range(9):
            params.append(torch.rand(1,dtype=cfg.global_args.torch_dtype,device=cfg.global_args.device)[0])
    bond_dim = args.bond_dim
    GTNO, cs = G_Ising(params[0:9])
    GTNO2, cs2 = G_Ising(params[9:18])
    ...
    A= torch.einsum('ijklmn,jabcd->ikalbmcnd',GTNO2,A).reshape(model.phys_dim,4*bond_dim,4*bond_dim,4*bond_dim,4*bond_dim)
    A= A/A.norm()
    ...
    print(cs[0].item(), cs[1].item(), cs[2].item(), cs[3].item(), cs[4].item(),\
          cs[5].item(), cs[6].item(), cs[7].item(), cs[8].item(), cs[9].item(), sum(i*i for i in cs).item())
    print(cs2[0].item(), cs2[1].item(), cs2[2].item(), cs2[3].item(), cs2[4].item(),\
          cs2[5].item(), cs2[6].item(), cs2[7].item(), cs2[8].item(), cs2[9].item(), sum(i*i for i in cs2).item())
    ...
    A2= torch.einsum('ijklmn,jabcd->ikalbmcnd',GTNO2,A2).reshape(model.phys_dim,4*bond_dim,4*bond_dim,4*bond_dim,4*bond_dim)
    ...
    GTNO2Z4 = torch.einsum('ijklmn,ka,lb,mc,nd->ijabcd',GTNO2,Z,Z,Z,Z)
    AZ4= torch.einsum('ijklmn,jabcd->ikalbmcnd',GTNOZ4,A).reshape(model.phys_dim,2*bond_dim,2*bond_dim,2*bond_dim,2*bond_dim)
    AZ4= torch.einsum('ijklmn,jabcd->ikalbmcnd',GTNO2Z4,AZ4).reshape(model.phys_dim,4*bond_dim,4*bond_dim,4*bond_dim,4*bond_dim)
    AZ4= AZ4/AZ4.norm()
    VOP = (XA-AZ4).norm()
    print (VOP.item())
    ...
    @torch.no_grad()
    def obs_fn(state_sym, ctm_env, params, opt_context):
